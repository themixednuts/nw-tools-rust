...
pub mod data_serializer {
    ...
    pub struct IntSerializer<T> {
        pub data: Vec<u8>,
        pub marker: PhantomData<T>,
    }

    impl<T: ToString> ToString for IntSerializer<T> {
        fn to_string(&self) -> String {
            let size_of = std::mem::size_of::<T>();
            assert_eq!(self.data.len(), size_of);
            match size_of {
                1 => i8::from_le_bytes(self.data.as_slice().try_into().unwrap()).to_string(),
                2 => i16::from_le_bytes(self.data.as_slice().try_into().unwrap()).to_string(),
                4 => i32::from_le_bytes(self.data.as_slice().try_into().unwrap()).to_string(),
                8 => i64::from_le_bytes(self.data.as_slice().try_into().unwrap()).to_string(),
                _ => unreachable!(),
            }
        }
    }
    #[derive(Serialize, Deserialize)]
    pub struct UIntSerializer<T> {
        pub data: Vec<u8>,
        pub marker: PhantomData<T>,
    }

    impl<T: ToString> ToString for UIntSerializer<T> {
        fn to_string(&self) -> String {
            let size_of = std::mem::size_of::<T>();
            assert_eq!(self.data.len(), size_of);
            match size_of {
                1 => u8::from_le_bytes(self.data.as_slice().try_into().unwrap()).to_string(),
                2 => u16::from_le_bytes(self.data.as_slice().try_into().unwrap()).to_string(),
                4 => u32::from_le_bytes(self.data.as_slice().try_into().unwrap()).to_string(),
                8 => u64::from_le_bytes(self.data.as_slice().try_into().unwrap()).to_string(),
                _ => unreachable!(),
            }
        }
    }
    ...
    #[derive(Serialize, Deserialize)]
    pub struct FloatSerializer<T> {
        pub data: Vec<u8>,
        pub marker: PhantomData<T>,
    }

    impl<T: ToString> ToString for FloatSerializer<T> {
        fn to_string(&self) -> String {
            let size_of = std::mem::size_of::<T>();
            assert_eq!(self.data.len(), size_of);
            match size_of {
                4 => f32::from_le_bytes(self.data.as_slice().try_into().unwrap()).to_string(),
                8 => f64::from_le_bytes(self.data.as_slice().try_into().unwrap()).to_string(),
                _ => unreachable!(),
            }
        }
    }
    ...
    // impl ToString for BoolSerializer {
    //     fn to_string(&self) -> String {
    //         let size_of = std::mem::size_of::<bool>();
    //         assert_eq!(self.data.len(), size_of);
    //     }
    // }
}
