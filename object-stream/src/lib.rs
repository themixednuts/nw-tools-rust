...
fn read_element(
    ...
    let mut buf = [0; 1];
    reader.read_exact(&mut buf)?;
    let flags = u8::from_be_bytes(buf);
    ...
    let mut buf = [0; 16];
    reader.read_exact(&mut buf)?;
    element._type = Uuid::from_slice(&buf)?.braced().to_string().to_uppercase();

    if stream.version == 2 {
        reader.read_exact(&mut buf)?;
        element.specialized_type =
            Some(Uuid::from_slice(&buf)?.braced().to_string().to_uppercase());
    }
